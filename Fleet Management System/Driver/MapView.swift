            ...
            Task {
                ...
                if let userId = IFEDataController.shared.user?.id {
                    ...
                    if let task = maintenanceTask {
                        ...
                        await tripViewModel.sendMessage(to: personnel.id, title: "Driver Emergency Alert", message: "Driver reported emergency at location: \(viewModel.currentLocationAddress). Current coordinates: \(viewModel.userLocation?.latitude ?? 0), \(viewModel.userLocation?.longitude ?? 0)")
                        
                        // Sending message to Fleet Manager
                        await tripViewModel.sendMessage(to: trip.assignedByFleetManagerID, title: "Driver Emergency Alert", message: "Driver reported emergency at location: \(viewModel.currentLocationAddress). Current coordinates: \(viewModel.userLocation?.latitude ?? 0), \(viewModel.userLocation?.longitude ?? 0)")
                    } else {
